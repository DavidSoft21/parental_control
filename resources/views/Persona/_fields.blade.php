    ...
    <select class="form-control select2" name="personas[madre_id]" id="{{PERSON_FIELD_MOTHER_ID}}">
        ...
        @foreach ($madres as $madre)
            @if($loop->first)
                <option value="">Seleccionar...</option>
            @else
                <option value="{{ $madre->id }}" {{old('persona.madre_id', isset($madre->id) ? $madre->id : '') == $madre->madre_id ? ' selected' : '' }}> {{ $madre->id }} - {{ $madre->primer_nombre}} , {{ $madre->primer_apellido}} </option>
            @endif
        @endforeach
    </select>
</div>
...
<div class="form-group col-md-6">
    ...
    <option value="">Seleccionar...</option>
        @foreach ($padres as $padre)
            <option value="{{ $padre->id }}" {{old('persona.padre_id', isset($padre->id) ? $padre->id : '') == $padre->padre_id ? ' selected' : '' }}> {{ $padre->id }} - {{ $padre->primer_nombre}} , {{ $padre->primer_apellido}} </option>
        @endforeach
    </select>
</div>
